...
namespace BackupAes256.ViewModel
{
    ...
    public class ViewModelBase : INotifyDataErrorInfo, INotifyPropertyChanged
    {
        ...
        public ViewModelBase()
        {
            ...
            dcClose = new DelegateCommand(ExecuteClose);
            dcIsClosing = new DelegateCommand(ExecuteIsClosing, CanExecuteIsClosing);
            dcCancelClosing = new DelegateCommand(ExecuteCancelClosing);
            dcWasClosed = new DelegateCommand(ExecuteWasClosed);
        }
        #endregion
        ...
        public void IterateNotifyPropertyChanged(string sFormat, int iFrom, int iTo)
        {
            for (int iIndex = iFrom; iIndex <= iTo; iIndex++)
                NotifyPropertyChanged(string.Format(sFormat, iIndex));
        }
